...
!(function (win, undefined) {
  ...
  xxim.tabs = function (index) {
    ...
    node.tabs
      .eq(index)
      .addClass('xxim_tabnow')
      .siblings()
      .removeClass('xxim_tabnow')
  ...
  xxim.expend = function () {
    ...
    if (xxim.layimNode.attr('state') !== '1') {
      xxim.layimNode
        .stop()
        .animate({ right: config.right }, config.aniTime, function () {
  ...
  xxim.popchat = function (param) {
    ...
    log.success = function (layero) {
      ...
      log.chatlist.on('click', 'li em', function (e) {
        ...
        if (parents.hasClass('layim_chatnow')) {
          ...
          xxim.tabchat(
            config.chating[
              chatlist.eq(indexs).attr('type') +
                chatlist.eq(indexs).attr('data-id')
            ]
          )
        }
    ...
    log.html =
      '<div class="layim_chatbox" id="layim_chatbox">' +
      '<h6>' +
      '<span class="layim_move"></span>' +
      '    <a href="' +
      param.url +
      '" class="layim_face" target="_blank"><img src="' +
      param.face +
      '" ></a>' +
      '    <a href="' +
      param.url +
      '" class="layim_names" target="_blank">' +
      param.name +
      '</a>' +
      ...
      '        <ul class="layim_chatview layim_chatthis"  id="layim_area' +
      param.type +
      param.id +
      '"></ul>' +
      '    </div>' +
    ...
    if (param.type === 'group') {
      ...
      log.chatgroup.append(
        '<ul class="layim_groupthis" id="layim_group' +
          param.type +
          param.id +
          '"></ul>'
      )
  ...
  xxim.tabchat = function (param) {
    ...
    xxim.chatbox
      .find('#layim_user' + keys)
      .addClass('layim_chatnow')
      .siblings()
      .removeClass('layim_chatnow')
    xxim.chatbox
      .find('#layim_area' + keys)
      .addClass('layim_chatthis')
      .siblings()
      .removeClass('layim_chatthis')
    xxim.chatbox
      .find('#layim_group' + keys)
      .addClass('layim_groupthis')
      .siblings()
      .removeClass('layim_groupthis')
    ...
    xxim.chatbox
      .find('.layim_seechatlog')
      .attr('href', config.chatlogurl + param.id)
        ...
        setTimeout(function () {
          log.imarea.append(
            log.html({
              ...
              content:
                config.autoReplay[
                  (Math.random() * config.autoReplay.length) | 0
                ]
            })
          )
          log.imarea.scrollTop(log.imarea[0].scrollHeight)
  ...
  xxim.event = function () {
    ...
    node.list.on('click', 'h5', function () {
      var othis = $(this),
        chat = othis.siblings('.xxim_chatlist'),
    ...
    node.xximSearch.keyup(function () {
      ...
      if (val !== '') {
        ...
        node.list
          .eq(3)
          .html('<li class="xxim_errormsg">没有符合条件的结果</li>')
      } else {
            ...
            if (index !== 2) {
              for (; i < myflen; i++) {
                str +=
                  ...
                  '<h5><i class="fa fa-caret-right"></i><span class="xxim_parentname">' +
                  datas.data[i].name +
              ...
              str += '<li class="xxim_liston">' + '<ul class="xxim_chatlist">'
              for (; i < myflen; i++) {
                str +=
                  '<li data-id="' +
                  datas.data[i].id +
  ...
  xxim.view = (function () {
    var xximNode = (xxim.layimNode = $(
      '<div id="xximmm" class="xxim_main">' +
        ...
        '</div>' +
        '</li>' +
        '<li class="xxim_mymsg" id="xxim_mymsg" title="我的私信"><i class="fa fa-comment"></i><a href="' +
        config.msgurl +
        '" target="_blank"></a></li>' +
