var App = (function () {
  ...
  function _drawBg() {
    ...
    img_bg.onload = function () {
      ...
      d3.timer(function () {
        // 圆上每个点的坐标
        var x =
          a - Math.cos(((2 * Math.PI) / 360) * angle + Math.acos(a / r)) * r
        var y =
          b - Math.sin(((2 * Math.PI) / 360) * angle + Math.asin(b / r)) * r
  ...
  function _drawProgressBar() {
    // top-right
    var progressLeftBar = d3.selectAll(
      '.progress-bar-bl-content > .progress-left-bar'
    )
    ...
    switch (menuNo) {
      case 0:
        ...
        for (var i = 0; i < itemMenu.length; i++) {
          html =
            html +
            '<div class="cmd"><a href="" class="cmd-label">' +
            itemMenu[i] +
            '</a></div>'
        }
        break
